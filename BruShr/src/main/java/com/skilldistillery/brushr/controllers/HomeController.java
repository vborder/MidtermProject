...
public class HomeController {
	...
	public String beerById(@RequestParam(name = "id") int id, Model model) {
		...
		BeerRecipe beer = beerDAO.getBeerById(id);// will have to match daoimpl
	...
	public String createBeer(Model model) {
		BeerRecipe beer = beerDAO.createBeer();
		model.addObject("beer", beer);
	...
	public String getAllBeers(@RequestParam(name = "FIXME") String beerByKeyword, Model model) {
		...
		if (beerByKeyword != "") {
			List<BeerRecipe> beers = beerDAO.searchBeerByKeyword(beerByKeyword);
			if (beers != null) {
				model.addObject("beers", beers);
				return "beerlist";
			} else {
	...
	public String getBeersByNameOrDscription(Model model) {
		List<BeerRecipe> beerlist = beerDAO.getBeersByNameOrDscription();//did we pass in an id
